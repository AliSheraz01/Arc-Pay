...
type TabType = 'overview' | 'schedule' | 'bulk' | 'templates' | 'batches'
...
export default function PayoutsPage() {
  ...
  const [duplicateGroups, setDuplicateGroups] = useState<{ address: string; name: string; count: number; totalAmount: number }[]>([])
  ...
  const parseCSVFile = (file: File) => {
    ...
    reader.onload = (event) => {
      ...
      const addressAmounts: Record<string, number> = {}
      ...
      for (let i = startIndex; i < lines.length; i++) {
        ...
        addressCount[lowerAddr] = (addressCount[lowerAddr] || 0) + 1
        addressAmounts[lowerAddr] = (addressAmounts[lowerAddr] || 0) + parseFloat(amountStr)
        if (!addressNames[lowerAddr]) addressNames[lowerAddr] = name
      ...
      // Detect duplicates
      const dups = Object.entries(addressCount)
        .filter(([, count]) => count > 1)
        .map(([addr]) => ({
          address: addr,
          name: addressNames[addr] || addr.slice(0, 6) + '...' + addr.slice(-4),
          count: addressCount[addr],
          totalAmount: addressAmounts[addr],
        }))
      ...
      if (errors.length > 0) {
        ...
        showToast(`${dups.length} duplicate recipient(s) found — please resolve.`, 'warning')
      } else {
        showToast(`CSV validated: ${parsed.length} recipients ready.`, 'success')
      }
    }
    reader.readAsText(file)
  }

  // Merge all duplicate entries into one row per address
  const handleMergeDuplicates = () => {
    const mergedMap: Record<string, Recipient> = {}
    csvRecipients.forEach(r => {
      const key = r.address.toLowerCase()
      if (mergedMap[key]) {
        mergedMap[key].amount = (parseFloat(mergedMap[key].amount) + parseFloat(r.amount)).toFixed(6)
      } else {
        mergedMap[key] = { ...r }
      }
    })
    const merged = Object.values(mergedMap)
    const total = merged.reduce((s, r) => s + parseFloat(r.amount), 0)
    setCsvRecipients(merged)
    setTotalCsvUSDC(total.toFixed(2))
    setCsvWarnings([])
    setDuplicateGroups([])
    setShowDuplicateModal(false)
    showToast(`Merged into ${merged.length} unique recipients.`, 'success')
  }

  // Keep all rows as-is (separate payments)
  const handleKeepSeparate = () => {
    setDuplicateGroups([])
    setShowDuplicateModal(false)
    showToast(`Keeping ${csvRecipients.length} separate payment rows.`, 'success')
  }
        ...
        <NetworkGuard>
          ...
          {showDuplicateModal && (
            <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '16px' }}>
              <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: '24px', padding: '28px', maxWidth: '480px', width: '100%', boxShadow: '0 20px 40px rgba(0,0,0,0.5)' }}>
                <h3 style={{ fontSize: '18px', fontWeight: 800, color: 'var(--text-primary)', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                  <MdWarning size={20} color="var(--yellow)" /> Duplicate Recipients Found
                </h3>
                <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginBottom: '20px', lineHeight: 1.6 }}>
                  The following {duplicateGroups.length} address(es) appear multiple times. Choose how to handle them:
                </p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '200px', overflowY: 'auto', marginBottom: '20px' }}>
                  {duplicateGroups.map(g => (
                    <div key={g.address} style={{ background: 'var(--surface-raised)', border: '1px solid var(--border)', borderRadius: '12px', padding: '12px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        <div style={{ fontSize: '13px', fontWeight: 700, color: 'var(--text-primary)', fontFamily: 'monospace' }}>{g.address.slice(0, 10)}...{g.address.slice(-8)}</div>
                        <div style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>{g.name} · {g.count} entries</div>
                      </div>
                      <div style={{ fontSize: '14px', fontWeight: 900, color: 'var(--accent)' }}>{g.totalAmount.toFixed(2)} USDC</div>
                    </div>
                  ))}
                </div>
                <div style={{ display: 'flex', gap: '12px' }}>
                  <button
                    ...
                    style={{ flex: 1, background: 'var(--accent)', border: 'none', borderRadius: '12px', padding: '12px', color: 'white', fontSize: '13px', fontWeight: 800, cursor: 'pointer' }}
                  >
                    Merge (Sum Amounts)
                  </button>
                  <button
                    onClick={handleKeepSeparate}
                    style={{ flex: 1, background: 'transparent', border: '1px solid var(--border)', borderRadius: '12px', padding: '12px', color: 'var(--text-primary)', fontSize: '13px', fontWeight: 700, cursor: 'pointer' }}
                  >
                    Keep Separate
                  </button>
                </div>
